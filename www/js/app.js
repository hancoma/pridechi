...
function json_call(reg_id) {
      ...
      // gcm reg_id 등록  
         $.post("http://m.pridechi.com/gcm_reg_app.php",
   {
    reg_id:reg_id,
    deviceid:deviceid
   },
...
function left_menu() {
     ...
     $( document ).ready(function() {
                    $.post("http://m.pridechi.com/left_menu.php",
               {
...
function goHref(url) {
  ...
  url="http://m.pridechi.com"+url+"&uuid="+uuid;
...
function getpage(uuid,page) {
    ...
    var uuid=uuid;
    var page=page;
    var url="http://m.pridechi.com/"+page+".php";
 $.post(url,
   {
...
function check_uuid (deviceid) {
        var deviceid=deviceid;
                     ...
                     $.post("http://m.pridechi.com/check_uuid_app.php",
       {
                deviceid:deviceid
       },
       function(data){
...
function onConfirm(buttonIndex) {
    ...
    if (btn==1) {
      ...
      $.post("http://m.pridechi.com/logout_app.php",
       {
                deviceid:uuid
       },
       function(data){
...
sourceType: navigator.camera.PictureSourceType.PHOTOLIBRARY
    ...
    function uploadPhoto_photo(imageURI) {
      var deviceid=device.uuid;
        ...
        ft.upload(imageURI, "http://m.pridechi.com/upload_org.php", win_photo, fail, options);
    }
...
function showPosition(position)
  ...
  if (x) {
   $.post("http://m.pridechi.com/gps_update_app.php",
   {
    y:y,
    x:x,
    uuid:uuid
...
right_menu();
 ...
 $( document ).ready(function() {
  ...
  $.post("http://m.pridechi.com/right_menu_app.php",
   {
      deviceid:deviceid
